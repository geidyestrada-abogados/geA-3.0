...
const apiURLoffice = "https://www.geidyestrada.com/api/office";
...
async function fetchOfficeContent() {
  try {
    ...
    if (officeData.Office && officeData.Office.url) {
      const officeUrl = `https://www.geidyestrada.com${officeData.Office.url}`;
      const baseUrl = "https://www.geidyestrada.com"; // Cambia esto si usas otra URL base para Strapi
...
async function loadOfficeContent() {
  try {
    const response = await fetch("https://www.geidyestrada.com/api/office?populate=*");
    if (!response.ok) {
      throw new Error(
    ...
    if (data && data.data) {
      ...
      if (officeData.Office && officeData.Office.length > 0) {
        ...
        const fullOfficeUrl = officeUrl.startsWith("http")
          ? officeUrl
          : `https://www.geidyestrada.com${officeUrl}`;
...
async function fetchOfficeBackgrounds() {
  try {
    const response = await fetch(
      "https://www.geidyestrada.com/api/office?populate=Office"
    );
...
function updateOfficePreview() {
  ...
  if (previewElement && selectedOfficeUrl) {
    previewElement.style.backgroundImage = `url(https://www.geidyestrada.com${selectedOfficeUrl})`;
...
async function saveOffice() {
  ...
  try {
    ...
    const response = await fetch("https://www.geidyestrada.com/api/office", {
      method: "PUT",
      headers: {
...
function updateOfficeBackground() {
  ...
  if (officeElement && selectedOfficeUrl) {
    officeElement.style.backgroundImage = `url(https://www.geidyestrada.com${selectedOfficeUrl})`;

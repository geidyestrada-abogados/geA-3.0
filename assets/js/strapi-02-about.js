...
async function loadAboutContent() {
  ...
  try {
    const response = await fetch("https://automatic-cheese-6aca9a943b.strapiapp.com/api/about");
    if (!response.ok) {
      throw new Error(
...
async function fetchAboutGeidypic() {
  try {
    const response = await fetch(
      "https://automatic-cheese-6aca9a943b.strapiapp.com/api/about?populate=Geidypic"
    );
    ...
    if (geidypicImages && geidypicImages.length > 0) {
      ...
      const firstImageUrl = `https://automatic-cheese-6aca9a943b.strapiapp.com${geidypicImages[0].url}`;
...
function updateGeidypicPreview() {
  ...
  if (previewElement) {
    previewElement.style.backgroundImage = `url(https://automatic-cheese-6aca9a943b.strapiapp.com${geidypicImageUrl})`;

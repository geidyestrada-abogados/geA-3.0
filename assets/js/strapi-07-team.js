    ...
    if (data && data.data) {
      ...
      for (let i = 1; i <= 4; i++) {
        ...
        if (teamPic?.url && imageElement) {
          imageElement.src = `https://automatic-cheese-6aca9a943b.strapiapp.com${teamPic.url}`;
...
async function loadTeamContent() {
  ...
  try {
    ...
    const response = await fetch("https://automatic-cheese-6aca9a943b.strapiapp.com/api/team?populate=*");
    if (!response.ok) {
      throw new Error(
